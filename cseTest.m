  ...
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	...
	while( bgRun ){
	  double t0;
		...
		[csex callLockedBlock:^{
		  double t1 = HRTime_toc();
		  unsigned long *nn = &n;
			// n is readonly here, but one can still change it via an indirection:
			*nn += 1;
			fprintf( stderr, "## got csex lock #%lu=%d at t=%g after %gs; starting %g s wait\n",
					n, [csex isLocked], t1, t1-t0, SLEEPTIMEBG ); fflush(stderr);
#ifndef BUSYSLEEPING
			MMSleep(SLEEPTIMEBG);
#else
			do{
...
{ char test[256], *head;
	...
	csex = [NSCriticalSection createWithSpinMax:4000];
	if( !csex ){
		fprintf( stderr, "Failure creating CSEHandle\n" );
		exit(1);
	}
	else{
		fprintf( stderr, "Created a '%s' NSCriticalSection with spinMax==%lu\n", [[[csex info] description] UTF8String], [csex spinMax] );
		[csex setInfo:@"bla"];
...
//		Sleep(1);
			...
			{ NSCriticalSectionScope *scope = [csex getLockScope];
			  double t1 = HRTime_toc();
				...
				fprintf( stderr, "> got csex lock #%d=%d at t=%g after %gs; starting %g s wait\n",
						rd->cycles, [csex isLocked], t1, t1-t0, SLEEPTIMEFG ); fflush(stderr);
#ifdef BUSYSLEEPING
				MMSleep(SLEEPTIMEFG);
#else
				do{
				...
				fprintf( stderr, "\tmain loop wait #%d ended at t=%gs; csex lock=%d\n",
					rd->cycles, HRTime_toc(), [csex isLocked] ); fflush(stderr);
				// an autoreleased scope instance would get released at some unknown point, probably NOT
	...
	// don't release csex if created with createWithSpinMax!!
	//[csex release];
	fprintf( stderr, "Draining pool\n" );
	[pool drain];
	exit(0);
}
