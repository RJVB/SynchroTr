...
REM runDoxygen.sh
REM
REM Created by René J.V. Bertin on 20120714.
...
SET DEST_NAME=SynchroTr.docset

echo We are in %~dp0 (%CD%)
...
REM echo "DOCSET_BUNDLE_ID       = org.RJVB.SynchroTr" >> %~dp0\doxygentmp.config

echo GENERATE_DOCSET        = YES >> %~dp0\doxygentmp.config
REM echo EXCLUDE_PATTERNS += Something >> %~dp0\doxygentmp.config
